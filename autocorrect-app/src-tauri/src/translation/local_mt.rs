...
#[cfg(not(feature = "local-translation"))]
use std::path::Path;
#[cfg(not(feature = "local-translation"))]
use std::sync::Arc;
...
mod imp {
    ...
    fn load_tokenizer(tokenizer_path: &Path) -> Result<Tokenizer> {
        let tokenizer_bytes = std::fs::read(tokenizer_path)
            .map_err(|e| Error::Config(format!("local MT read tokenizer: {e}")))?;
        let mut tokenizer_json: Value = serde_json::from_slice(&tokenizer_bytes)
            .map_err(|e| Error::Config(format!("local MT tokenizer JSON: {e}")))?;
        ...
        let tokenizer_bytes = if sanitized {
            serde_json::to_vec(&tokenizer_json)
                .map_err(|e| Error::Config(format!("local MT tokenizer rewrite: {e}")))?
        } else {
            tokenizer_bytes
        };
        let tokenizer = std::panic::catch_unwind(|| Tokenizer::from_bytes(&tokenizer_bytes))
            ...
            .map_err(|e| Error::Config(format!("local MT tokenizer: {e}")))?;
        if sanitized {
            log::warn!(
    ...
    impl MarianBackend {
        fn new(model_id: &str, dir: &Path, source_lang: &str) -> Result<Self> {
            ...
            let encoder = Session::builder()
                .map_err(|e| Error::Config(format!("local MT encoder builder: {e}")))?
                .with_intra_threads(2)
                .map_err(|e| Error::Config(format!("local MT encoder threads: {e}")))?
                .commit_from_file(&encoder_path)
                .map_err(|e| Error::Config(format!("local MT load encoder: {e}")))?;
            let decoder = Session::builder()
                .map_err(|e| Error::Config(format!("local MT decoder builder: {e}")))?
                .with_intra_threads(2)
                .map_err(|e| Error::Config(format!("local MT decoder threads: {e}")))?
                .commit_from_file(&decoder_path)
                .map_err(|e| Error::Config(format!("local MT load decoder: {e}")))?;
        ...
        fn encode_text(&self, text: &str) -> Result<Vec<i64>> {
            let encoding = self
                .tokenizer
                .encode(text, true)
                .map_err(|e| Error::Config(format!("local MT encode: {e}")))?;
        ...
        fn run_encoder(&self, input_ids: &[i64]) -> Result<ArrayD<f32>> {
            ...
            let ids_arr = Array2::from_shape_vec((1, seq_len), input_ids.to_vec())
                .map_err(|e| Error::Config(format!("local MT encoder shape: {e}")))?;
            let mask_arr = Array2::<i64>::ones((1, seq_len));

            let ids_tensor = Tensor::from_array(ids_arr)
                .map_err(|e| Error::Config(format!("local MT ids tensor: {e}")))?;
            let mask_tensor = Tensor::from_array(mask_arr)
                .map_err(|e| Error::Config(format!("local MT mask tensor: {e}")))?;

            let mut encoder = self.encoder.lock().unwrap();
            let outputs = encoder
                .run(inputs!["input_ids" => ids_tensor, "attention_mask" => mask_tensor])
                .map_err(|e| Error::Config(format!("local MT encoder run: {e}")))?;

            let (shape, data) = outputs["last_hidden_state"]
                .try_extract_tensor::<f32>()
                .map_err(|e| Error::Config(format!("local MT hidden state: {e}")))?;
            let dims: Vec<usize> = shape.iter().map(|&d| d as usize).collect();
            ArrayD::from_shape_vec(IxDyn(&dims), data.to_vec())
                .map_err(|e| Error::Config(format!("local MT hidden reshape: {e}")))
        }
        ...
        fn greedy_decode(
            ...
            for _ in 0..DEFAULT_MAX_LENGTH {
                ...
                .map_err(|e| Error::Config(format!("local MT decoder run: {e}")))?;

                let (shape, logits) = outputs["logits"]
                    .try_extract_tensor::<f32>()
                    .map_err(|e| Error::Config(format!("local MT logits: {e}")))?;
        ...
        fn decode_ids(&self, ids: &[i64]) -> Result<String> {
            ...
            self.tokenizer
                .decode(&ids, true)
                .map_err(|e| Error::Config(format!("local MT decode: {e}")))
        }
    }
...
#[cfg(feature = "local-translation")]
pub use imp::{load_backend, model_files_ready};

#[cfg(feature = "local-translation")]
static BACKEND_CACHE: std::sync::OnceLock<
    std::sync::Mutex<
        std::collections::HashMap<(std::path::PathBuf, String), std::sync::Arc<dyn LocalMtBackend>>,
    >,
> = std::sync::OnceLock::new();

#[cfg(feature = "local-translation")]
fn backend_cache() -> &'static std::sync::Mutex<
    std::collections::HashMap<(std::path::PathBuf, String), std::sync::Arc<dyn LocalMtBackend>>,
> {
    BACKEND_CACHE.get_or_init(|| std::sync::Mutex::new(std::collections::HashMap::new()))
}

/// Load a local MT backend, caching the result by model path and source language.
#[cfg(feature = "local-translation")]
pub fn load_backend_cached(
    model_path: &std::path::Path,
    source_lang: Option<&str>,
) -> Result<std::sync::Arc<dyn LocalMtBackend>> {
    let source_key = source_lang.unwrap_or("auto").to_string();
    let key = (model_path.to_path_buf(), source_key);
    {
        let cache = backend_cache()
            .lock()
            .map_err(|e| Error::Config(format!("local MT cache lock: {e}")))?;
        if let Some(backend) = cache.get(&key) {
            return Ok(std::sync::Arc::clone(backend));
        }
    }
    let backend = load_backend(model_path, source_lang)?;
    {
        let mut cache = backend_cache()
            .lock()
            .map_err(|e| Error::Config(format!("local MT cache lock: {e}")))?;
        cache.insert(key, std::sync::Arc::clone(&backend));
    }
    Ok(backend)
}
...
pub fn load_backend_cached(
    ...
    load_backend(model_path, source_lang)
}
...
#[cfg(not(feature = "local-translation"))]
pub fn provider_requires_source_language(_model_path: &std::path::Path) -> bool {
    false
}

#[cfg(not(feature = "local-translation"))]
pub fn model_files_ready(_model_path: &std::path::Path) -> bool {
    false
}
